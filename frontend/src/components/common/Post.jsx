...
const Post = ({ post }) => {
  ...
  const { mutate: deletePost, isPending: isDeleting } = useMutation({
    mutationFn: () => deletePostById(post._id),
    onSuccess: () => {
  ...
  const { mutate: toggleLikePost, isPending: isLiking } = useMutation({
    mutationFn: () => toggleLike(post._id),
    onSuccess: () => {
      queryClient.setQueryData(["posts"], (oldPosts) => {
        return oldPosts.map((oldPost) => {
          if (oldPost._id !== post._id) return oldPost;
          const hasLiked = oldPost.likes.includes(authUser._id);
          return {
            ...oldPost,
            likes: hasLiked
              ? oldPost.likes.filter((el) => el !== authUser._id)
